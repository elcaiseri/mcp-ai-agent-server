...
class AIAgentMCPClient:
    ...
    async def _initialize_agent(self):
        ...
        try:
            ...
            resources_per_server = await asyncio.gather(
                *(
                    self.mcp_client.get_resources(name)
                    for name in self.mcp_client.connections.keys()
                )
            )
            # Flatten the list of resources
            self.resources = [
                item for sublist in resources_per_server for item in sublist
            ]
    ...
    async def agent_conversation(
        self, user_input: str, history: Optional[List[Dict[str, str]]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _display_tool_calls(
        ...
        tool_calls_tree = Tree(
            Text("🔧 Tool Execution", style="bold cyan"), guide_style="bright_blue"
        )
    ...
    def _export_conversation(self, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.console.print(
                f"[green]✓ Conversation exported to:[/green] {filename}\n"
            )
    ...
    async def chat(self, callback=None) -> None:
        ...
        if not self.agent:
            self.console.print(
                Panel(
                    Text(
                        "❌ AI Agent not initialized. Please set OPENAI_API_KEY.",
                        style="bold red",
                    ),
                    border_style="red",
                    box=box.HEAVY,
        ...
        while True:
            try:
                ...
                if user_input.lower() in ["exit", "quit", "bye"]:
                    ...
                    goodbye_text.append(
                        f"  • Commands used: {len(self.command_history)}", style="white"
                    )
                ...
                if user_input.lower().startswith("debug"):
                    if "on" in user_input.lower():
                        ...
                        self.console.print(
                            "[green]✓ Debug mode enabled[/green] - Token stats will be shown\n"
                        )
                    elif "off" in user_input.lower():
                        self.show_stats = False
                        self.console.print(
                            "[yellow]✓ Debug mode disabled[/yellow] - Token stats hidden\n"
                        )
                    else:
                        status = (
                            "[green]enabled[/green]"
                ...
                with Progress(
                    ...
                    result = await self.agent_conversation(
                        user_input, result.get("history", None)
                    )
                ...
                if result["success"]:
                    ...
                    if result.get("history"):
                        self._display_tool_calls(
                            result["history"], previous_message_count
                        )
                    ...
                    error_text.append(
                        "Try rephrasing your question or check the logs", style="dim"
                    )

                    error_panel = Panel(
                        error_text, border_style="red", box=box.HEAVY, padding=(1, 2)
                    )
    ...
    mcp_client = MultiServerMCPClient(
        {
            "command-server": {
                "transport": "stdio",
                "command": "uv",
                "args": ["run", "python", "-m", "src.servers.command"],
            },
            "file-manager-server": {
                "transport": "stdio",
                "command": "uv",
                "args": ["run", "python", "-m", "src.servers.file_manager"],
            },
            "web-server": {
                "transport": "stdio",
                "command": "uv",
                "args": ["run", "python", "-m", "src.servers.web"],
            },
            "memory-server": {
                "transport": "stdio",
                "command": "uv",
                "args": ["run", "python", "-m", "src.servers.memory"],
            },
            "news-server": {
                "transport": "stdio",
                "command": "uv",
                "args": ["run", "python", "-m", "src.servers.news"],
            },
